...
@section('content_header')
    <div>
        <ol class="breadcrumb">
            ...
            <li class="breadcrumb-item">
                <a class="" href="{{ route('plans.show', $plan->url) }}">Planos</a>
            </li>
            <li class="breadcrumb-item active">
                <a class="" href="javascript:void(0)">Detalhes do plano {{$plan->name}}</a>
            </li>
        </ol>
        <h1 class="d-inline-flex mr-3" style="vertical-align: middle">Detalhes do plano {{$plan->name}}</h1>
        <a href="{{ route('details.plan.create',$plan->url) }}" class="btn btn-outline-info">
                ...
                <tbody>
                    @foreach($details as $detail)
                        <tr>
                            ...
                            <td style="width: 20%">
                                <a href="{{ route('detail.plan.show', [$plan->url, $detail->id]) }}" class="btn btn-outline-warning">
                                    <span class="fas fa-eye mr-2"></span> Ver
                                </a>
                                <a href="{{ route('detail.plan.edit', [$plan->url, $detail->id]) }}" class="btn btn-outline-primary">
