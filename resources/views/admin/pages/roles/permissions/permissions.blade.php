...
@section('title', "Permissões do função {$role->name}")
    ...
    <div>
        <ol class="breadcrumb">
            ...
            <li class="breadcrumb-item active">
                <a class="active" href="javascript:void(0)">Permissões do função {{ $role->name }}</a>
            </li>
        </ol>
        <h1 class="d-inline-flex mr-3" style="vertical-align: middle">Permissões do função {{ $role->name }}</h1>
        <a href="{{ route('roles.permissions.avaliable', $role->id) }}" class="btn btn-outline-info">
                ...
                <tbody>
                    @foreach($permissions as $permission)
                        <tr>
                            ...
                            <td style="width: 20%">
                                <a href="{{ route('roles.show', $role->id) }}" class="btn btn-outline-warning">
                                    <span class="fas fa-eye "></span>
                                </a>
                                <a href="{{ route('roles.edit', $role->id) }}" class="btn btn-outline-primary">
                                    <span class="fas fa-edit"></span>
                                </a>
                                <a href="{{ route('roles.permissions.detach', [$role->id, $permission->id]) }}" class="btn btn-outline-secondary">
